...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Clear existing data in safe order (delete individually for Djongo compatibility)
        ...
        Activity.objects.create(user=users[2], type='Lasso Practice', duration=30, date=timezone.now().date())
